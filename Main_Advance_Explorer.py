...
class FileExplorer(QMainWindow):
    ...
    def show_shortcuts_dialog(self):
        ...
        rows = [
            ...
            ("nav",   "Ctrl+더블클릭",      "윈도우 탐색기로 경로 열기", "선택 항목 또는 현재 폴더를 네이티브 윈도우 탐색기로 열기"),
    ...
    def add_tab(self, tab_widget, path, name, side):
        ...
        # 변경된 코드
        tree.customContextMenuRequested.connect(lambda pos: self.handle_context_menu(tree, pos, tab))
    ...
    def on_tree_double_clicked(self, tab, index, side):
        """트리뷰 더블클릭 - Ctrl+더블클릭 시 윈도우 탐색기 호출, 메모는 편집, 그 외는 열기"""
        # 1. Ctrl 키가 눌렸는지 감지 (수정된 부분)
        modifiers = QApplication.keyboardModifiers()
        is_ctrl_pressed = bool(modifiers & Qt.ControlModifier)

        # Ctrl + 더블클릭인 경우 윈도우 탐색기 열기 (수정된 부분)
        if is_ctrl_pressed and platform.system() == 'Windows':
            if index.isValid():
                source_index = tab.model.mapToSource(index)
            ...
            return  # 윈도우 탐색기를 띄웠으므로 기존 더블클릭 동작 취소

        # 2. 메모 컬럼(4번)인 경우 편집만 진행
        if index.column() == 4:
            return
        
        # 3. 다른 컬럼은 기존처럼 폴더 진입 또는 파일 열기
    ...
    def handle_context_menu(self, tree, pos, tab):
        """컨텍스트 메뉴 표시 (순수 우클릭 메뉴 기능만 담당)"""
        ...
        if index.isValid():
            ...
            # [추가] 선택된 항목을 윈도우 탐색기에서 열기
            if platform.system() == 'Windows':
                menu.addSeparator()
                explorer_action = QAction("🪟 윈도우 탐색기에서 열기", self)
                explorer_action.triggered.connect(lambda: self.open_in_windows_explorer(tab, index=index))
            ...
            # [추가] 빈 공간 우클릭 시 현재 위치를 윈도우 탐색기에서 열기
            if platform.system() == 'Windows':
                menu.addSeparator()
                explorer_empty_action = QAction("🪟 현재 위치 윈도우 탐색기에서 열기", self)
                explorer_empty_action.triggered.connect(lambda: self.open_in_windows_explorer(tab, index=None))
                menu.addAction(explorer_empty_action)
    ...
    def open_in_windows_explorer(self, tab, index=None):
        """선택된 항목 또는 현재 폴더를 네이티브 윈도우 탐색기로 엽니다."""
        if platform.system() != 'Windows':
            return

        if index and index.isValid():
            # 특정 항목(파일/폴더)을 우클릭한 경우: 해당 항목이 선택된 상태로 탐색기 열기
            source_index = tab.model.mapToSource(index)
            target_path = tab.source_model.filePath(source_index)
            subprocess.run(['explorer', '/select,', os.path.normpath(target_path)])
        else:
            # 빈 공간을 우클릭한 경우: 현재 열려있는 폴더를 탐색기에서 열기
            subprocess.run(['explorer', os.path.normpath(tab.current_path)])
